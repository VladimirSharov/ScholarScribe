...
CURATED_EXAMPLES = "boa_strangling/demo/curated_examples.json"
...
def main():
    ...
    # Hand-curated (from checkpoint_comparison_3way's seed42/n100 sample) so
    # each example demonstrates something specific: a rare checkpoint-117000
    # win, a long-tail-heavy thesis, a clean failure case, a micro- vs
    # macro-F1 illustration, and two self-authored synthetic texts. Tracked
    # in git (unlike the rest of package/) since it's small text, not a
    # binary artifact - edit CURATED_EXAMPLES directly rather than
    # regenerating it from this script.
    shutil.copy(CURATED_EXAMPLES, f"{OUT_DIR}/examples.json")
    with open(CURATED_EXAMPLES, encoding="utf-8") as f:
        print(f"Copied {len(json.load(f))} curated examples to examples.json")
