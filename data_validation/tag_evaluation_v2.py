...
def evaluate_tags(results_file):
    ...
    for item in results:
        ...
        # Get generated tags (already combined)
        generated_tags = item.get('generated_tags', [])
        
        # Calculate exact match metrics
        combined_exact = exact_match_ratio(original_combined, generated_tags)
        
        # Store metrics
        combined_exact_ratios.append(combined_exact)
        
        # Track perfect and zero matches
        if combined_exact == 1.0:
            metrics["summary_metrics"]["combined_tags"]["perfect_matches"] += 1
        if combined_exact == 0.0:
            metrics["summary_metrics"]["combined_tags"]["zero_matches"] += 1
        
        # Store detailed metrics for this item
        item_metrics = {
            "identifier": item.get("identifier", ""),
            "combined_tags_metrics": {
                "exact_match_ratio": combined_exact,
    ...
    # Calculate averages
    metrics["summary_metrics"]["combined_tags"]["exact_match_ratio_avg"] = np.mean(combined_exact_ratios) if combined_exact_ratios else 0
    
    # Add percentages
    total = len(results)
    metrics["summary_metrics"]["combined_tags"]["perfect_match_percentage"] = (metrics["summary_metrics"]["combined_tags"]["perfect_matches"] / total) * 100 if total else 0
    metrics["summary_metrics"]["combined_tags"]["zero_match_percentage"] = (metrics["summary_metrics"]["combined_tags"]["zero_matches"] / total) * 100 if total else 0
...
def create_visualization_data(metrics, output_path):
    ...
    viz_data = {
        ...
        "distribution": {
            "combined_exact_match": get_distribution(metrics["detailed_metrics"], 
...
def main():
    ...
    metrics_output = os.path.join(output_dir, "tag_evaluation_metrics.json")
    viz_output = os.path.join(output_dir, "tag_visualization_data.json")
    ...
    print("\nCombined Tags:")
    print(f"  Exact match ratio (avg): {metrics['summary_metrics']['combined_tags']['exact_match_ratio_avg']:.2f}")
    print(f"  Perfect matches: {metrics['summary_metrics']['combined_tags']['perfect_matches']} " +
          f"({metrics['summary_metrics']['combined_tags']['perfect_match_percentage']:.1f}%)")
    print(f"  Zero matches: {metrics['summary_metrics']['combined_tags']['zero_matches']} " +
          f"({metrics['summary_metrics']['combined_tags']['zero_match_percentage']:.1f}%)")
    
    print("\nExplanation:")
    print("  - Exact match ratio: The proportion of tags that match between original and generated sets")
    print("  - Perfect match: When ALL tags match perfectly (exact match ratio = 1.0)")
    print("  - Zero match: When NO tags match at all (exact match ratio = 0.0)")
