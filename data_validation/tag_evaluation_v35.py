...
def calculate_true_emd(source_embeddings, target_embeddings):
    """
    Computes the true Earth Mover's Distance using Optimal Transport (POT).
    Requires two equally weighted distributions over tag embeddings.
    """
    if len(source_embeddings) == 0 or len(target_embeddings) == 0:
        return np.nan

    # Normalize uniform weights (probability distributions)
    a = np.ones(len(source_embeddings)) / len(source_embeddings)
    b = np.ones(len(target_embeddings)) / len(target_embeddings)

    # Compute the cost matrix (e.g., cosine distances between all pairs)
    cost_matrix = cdist(source_embeddings, target_embeddings, metric='cosine')
...
def tag_similarity_analysis(results_data):
    ...
    for i, item in enumerate(results_data['results']):
        ...
        emd = calculate_true_emd(original_embeddings, generated_embeddings)
...
def visualize_results(analysis_results, output_dir):
    ...
    plt.ylabel('Count')
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(output_dir, 'emd_distribution.png'))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir,'emd_by_language.png'))
    ...
    plt.grid(alpha=0.3)
    plt.savefig(os.path.join(output_dir,'f1_vs_emd.png'))
    
    # Save results to CSV
    df.to_csv(os.path.join(output_dir, 'tag_similarity_analysis.csv'), index=False)
...
def analyze_tag_tokens(analysis_results, output_dir):
    ...
    # Save token analysis
    with open((os.path.join(output_dir,'tag_token_analysis.txt')), 'w') as f:
...
def main():
    # Load results data
    filepath = "generated_tags_results_union_tags.json"
    results_data = load_results(filepath)
    # Global timestamped output folder
    ...
    df = visualize_results(analysis_results, output_dir)
    
    # Analyze tag tokens
    print("Analyzing tag tokens...")
    analyze_tag_tokens(analysis_results, output_dir)
